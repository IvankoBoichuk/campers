    ...
    return (
        <div {...props}>
            ...
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger ref={ref} className="w-full bg-theme-inputs rounded-[12px] px-5 py-[18px] flex items-center gap-2">
                    <IconLocation className="fill-current" />
                    {value
                        ? (filters.location.find((location) => location.name === value)?.name ?? null)
                        : "Select Location"}
                </PopoverTrigger>
